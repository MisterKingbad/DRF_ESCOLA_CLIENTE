...
def criando_cursos(quantidade_de_cursos):
    ...
    for _ in range(quantidade_de_cursos):
        ...
        descrisao_curso = random.choice(descs)
        descs.remove(descrisao_curso)
        nivel_curso = random.choice("BIA")
        c = Curso(codigo_curso=codigo_curso,descrisao_curso=descrisao_curso, nivel_curso=nivel_curso)
        c.save()
